...
export default function SalaryTemplatesPage() {
  ...
  async function performDelete(id) {
    try {
      setAlertConfig(showLoading("Deleting template..."));
      const res = await fetch(`/api/salary-templates/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Failed to delete template");
      await fetchTemplates();
      setAlertConfig(showSuccessDelete("Template deleted successfully!", () => setAlertConfig(null)));
    } catch (err) {
      setAlertConfig(showError(err.message || "Failed to delete template", () => setAlertConfig(null)));
    }
  }
  ...
  return (
    <div>
      ...
      >
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-white">Salary Templates</h1>
          <p className="text-xs mt-0.5" style={{ color: '#9ca0c7' }}>{total} total templates</p>
        </div>
        <Link href="/salary-templates/new" className="btn-primary px-4 sm:px-5 py-2 rounded-full text-sm font-semibold flex items-center gap-2 shadow-lg">
