          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[700px]">
              <thead><tr style={{ background: 'var(--bg-header)', borderBottom: '1px solid var(--border-color)' }}>
                <th className="text-left px-5 py-3"><div className="h-3 w-28 rounded bg-gray-200 animate-pulse" style={{ background: 'var(--border-color)' }}></div></th>
                <th className="text-left px-5 py-3"><div className="h-3 w-32 rounded bg-gray-200 animate-pulse" style={{ background: 'var(--border-color)' }}></div></th>
                <th className="text-left px-5 py-3"><div className="h-3 w-20 rounded bg-gray-200 animate-pulse" style={{ background: 'var(--border-color)' }}></div></th>
                <th className="text-left px-5 py-3"><div className="h-3 w-28 rounded bg-gray-200 animate-pulse" style={{ background: 'var(--border-color)' }}></div></th>
                <th className="text-center px-5 py-3"><div className="h-3 w-14 rounded bg-gray-200 animate-pulse mx-auto" style={{ background: 'var(--border-color)' }}></div></th>
                <th className="text-right px-5 py-3"><div className="h-3 w-14 rounded bg-gray-200 animate-pulse ml-auto" style={{ background: 'var(--border-color)' }}></div></th>
              </tr></thead>
              <tbody>
                {[...Array(perPage || 10)].map((_, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid var(--border-light)' }}>
                    ...
                    <td className="px-5 py-4"><div className="h-3.5 w-36 rounded bg-gray-200 animate-pulse" style={{ background: 'var(--border-color)' }}></div></td>
                    ...
                    <td className="px-5 py-4"><div className="flex justify-end gap-2">{[...Array(3)].map((_, k) => <div key={k} className="w-7 h-7 rounded-lg bg-gray-200 animate-pulse" style={{ background: 'var(--border-color)' }}></div>)}</div></td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <table className="w-full min-w-[700px]">
                <thead>
                  <tr style={{ background: 'var(--bg-header)', borderBottom: '1px solid var(--border-color)' }}>
                    <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider cursor-pointer select-none hover-surface transition" style={{ color: 'var(--text-secondary)' }} onClick={() => handleSort("clientName")}><span className="inline-flex items-center">Client Name<SortIcon field="clientName" /></span></th>
                    <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider cursor-pointer select-none hover-surface transition" style={{ color: 'var(--text-secondary)' }} onClick={() => handleSort("email")}><span className="inline-flex items-center">Email<SortIcon field="email" /></span></th>
                    <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider cursor-pointer select-none hover-surface transition" style={{ color: 'var(--text-secondary)' }} onClick={() => handleSort("phone")}><span className="inline-flex items-center">Phone<SortIcon field="phone" /></span></th>
                    <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider cursor-pointer select-none hover-surface transition" style={{ color: 'var(--text-secondary)' }} onClick={() => handleSort("gstNumber")}><span className="inline-flex items-center">GST Number<SortIcon field="gstNumber" /></span></th>
                    ...
                    <th className="text-center px-5 py-3 text-xs font-semibold uppercase tracking-wider cursor-pointer select-none hover-surface transition" style={{ color: 'var(--text-secondary)' }} onClick={() => handleSort("isActive")}><span className="inline-flex items-center justify-center">Status<SortIcon field="isActive" /></span></th>
                ...
                <tbody>
                  {clients.length === 0 ? (
                    <tr>
                      <td colSpan={6}>
                        <NoResults
                          message={search ? `No clients found for "${search}"` : (stateFilter || cityFilter || locationFilter) ? "No clients match the selected filters" : "No clients added yet"}
                          onClear={(search || stateFilter || cityFilter || locationFilter) ? clearFilters : undefined}
                        />
                      </td>
                    </tr>
                  ) : clients.map((client) => (
                    <tr key={client._id} className="transition" style={{ borderBottom: '1px solid var(--border-light)' }}>
                      ...
                      <td className="px-5 py-4 text-center">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium status-badge ${client.isActive ? 'is-active' : 'is-inactive'}`}>
                      ...
                      <td className="px-5 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Link href={`/clients/${client._id}/view`} className="action-icon-btn" aria-label="View" style={{ '--tt-bg': '#e0f2fe', '--tt-fg': '#0284c7' }}>
                          ...
                          <Link href={`/clients/${client._id}`} className="action-icon-btn" aria-label="Edit" style={{ '--tt-bg': '#e0e7ff', '--tt-fg': '#6366f1' }}>
                          ...
                          <button onClick={() => handleDelete(client._id, client.clientName)} className="action-icon-btn" aria-label="Delete" style={{ '--tt-bg': '#fee2e2', '--tt-fg': '#ef4444' }}>
              ...
              {clients.length === 0 ? (
                ...
                <div key={client._id} className="p-4" style={{ borderColor: 'var(--border-light)' }}>
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium status-badge ${client.isActive ? 'is-active' : 'is-inactive'}`}>
                      <span className="w-1.5 h-1.5 rounded-full" style={{ background: client.isActive ? '#10b981' : '#ef4444' }}></span>
                      {client.isActive ? "Active" : "Inactive"}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-3 pt-3" style={{ borderTop: '1px solid var(--border-light)' }}>
                    <Link href={`/clients/${client._id}/view`} className="flex-1 text-center py-2 rounded-lg text-xs font-semibold" style={{ background: '#e0f2fe', color: '#0284c7' }}>View</Link>
